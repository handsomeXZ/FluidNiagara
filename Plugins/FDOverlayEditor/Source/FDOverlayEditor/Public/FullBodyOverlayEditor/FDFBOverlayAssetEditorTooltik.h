...
class FFDFBOverlayEditorModeUILayer;


class FDASSISTOR_API FFDFBOverlayAssetEditorToolkit : public FBaseAssetToolkit
{
public:
	FFDFBOverlayAssetEditorToolkit(UAssetEditor* InOwningAssetEditor);
	virtual ~FFDFBOverlayAssetEditorToolkit();
...
protected:
	...
	TWeakPtr<SEditorViewport> FDFBOverlay2DViewport;
	UFDAssistorViewportButtonsAPI* ViewportButtonsAPI = nullptr;

	UFDAssistorLive2DViewportAPI* FDAssistorLive2DViewportAPI = nullptr;

	TSharedPtr<FFDFBOverlayEditorModeUILayer> ModeUILayer;
	TSharedPtr<FWorkspaceItem> FDFBOverlayMenuCategory;
};