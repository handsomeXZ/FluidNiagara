...
#include "FDAssistorMeshInput.generated.h"
 ...
 */
UCLASS()
class FDASSISTOR_API UFDAssistorMeshInput : public UToolTarget
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnObjectModified, UFDAssistorMeshInput* InputObject, const FCanonicalModifiedInfo&);
	...
	bool InitializeMeshes(UToolTarget* Target, 
		...
		TFunction<FVector2f(const FVector3d&)> VertPositionToUVFuncIn) {return false;};

	void Shutdown() {};

	//注意以下便利函数:
	// 1。为ChangedVids / changnedelementids / ChangedConnectivityTids 传递 nullptr 意味着所有的 vids / elements / tids 都需要分别更新。
	//		传递 UFDAssistorMeshInput::NONE_CHANGED_ARG 相当于传递一个指向空数组的指针，意味着没有任何变化。否则，只有指向数组中的 vids / elements / tids 在更新的网格中迭代。
