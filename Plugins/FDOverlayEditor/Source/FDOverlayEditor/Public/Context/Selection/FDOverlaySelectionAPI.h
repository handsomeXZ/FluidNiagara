...
#include "Context/Selection/FDOverlaySelection.h"

#include "FDOverlaySelectionAPI.generated.h"
 ...
 */
UCLASS()
class FDOVERLAYEDITOR_API UFDOverlaySelectionAPI : public UFDOverlayContextObject
{
	GENERATED_BODY()
public:
...
//
	 ...
	 */
	enum class EFDEditorSelectionMode
	{
...
//	
//	// 可以让api把这些放到LivePreviewAPI委托中，但决定不让它与上面的Render()和DrawHUD()对齐。所有者会直接调用这些。
//	virtual void Live3DPreviewRender(IToolsContextRenderAPI* RenderAPI);
//	virtual void Live3DPreviewDrawHUD(FCanvas* Canvas, IToolsContextRenderAPI* RenderAPI);
//
...
// UInterface for IFDToolSupportsSelection
UINTERFACE(MinimalAPI)
class UFDToolSupportsSelection : public UInterface
{
	GENERATED_BODY()
};

/**
 * If a tool does not inherit from IFDToolSupportsSelection, then selection
 ...
 * If a tool does inherit from IFDToolSupportsSelection, then the UV editor will
 ...
 */
class IFDToolSupportsSelection
{
	GENERATED_BODY()
