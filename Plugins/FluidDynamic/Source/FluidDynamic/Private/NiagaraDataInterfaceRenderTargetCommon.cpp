		...
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
			[](const TArray<FString>& Args, UWorld*)
			{
				...
				if ( ensure(TextureRenderTargetFormatEnum) )
				{
					...
					UE_LOG(LogFluidFrid2DNiagara, Log, TEXT("Niagara RenderTarget Override is '%s' with format '%s'."), GNiagaraRenderTargetOverrideFormatEnabled ? TEXT("Enabled") : TEXT("Disabled"), *TextureRenderTargetFormatEnum->GetNameStringByIndex(int32(GNiagaraRenderTargetOverrideFormat)));
				}
			}
		)
	);
	...
	bool GetRenderTargetFormat(bool bOverrideFormat, ETextureRenderTargetFormat OverrideFormat, ETextureRenderTargetFormat& OutRenderTargetFormat)
	{
		...
		while (RHIIsTypedUAVStoreSupported(GetPixelFormatFromRenderTargetFormat(OutRenderTargetFormat)) == false)
		{
			...
			if (PreviousFormat == OutRenderTargetFormat)
			{
				...
				UE_LOG(LogFluidFrid2DNiagara, Warning, TEXT("Failed to find a render target format that supports UAV store"));
				return false;
			}
		}
