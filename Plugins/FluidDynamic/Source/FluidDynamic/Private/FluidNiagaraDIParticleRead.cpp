...
#include "FluidNiagaraDIParticleRead.h"
...
#define LOCTEXT_NAMESPACE "FluidNiagaraDIParticleRead"



struct FFluidNParticleReadDIFunctionVersion
{
	enum Type
	{
		InitialVersion = 0,
...
struct FFluidNDIParametersCS_ParticleRead : public FNiagaraDataInterfaceParametersCS
{
	DECLARE_TYPE_LAYOUT(FFluidNDIParametersCS_ParticleRead, NonVirtual);
...
IMPLEMENT_TYPE_LAYOUT(FFluidNDIParametersCS_ParticleRead);
...
struct FFluidNDIParticleRead_InstanceData
{
	FNiagaraSystemInstance* SystemInstance;
	FNiagaraEmitterInstance* EmitterInstance;
};

struct FFluidNDIParticleRead_GameToRenderData
{
	FFluidNDIParticleRead_GameToRenderData() : SourceEmitterGPUContext(nullptr) {}

	FNiagaraComputeExecutionContext* SourceEmitterGPUContext;
};

struct FFluidNDIParticleRead_RenderInstanceData
{
	FFluidNDIParticleRead_RenderInstanceData()
	{
...
struct FFluidNiagaraDIProxyParticleRead : public FNiagaraDataInterfaceProxyRW
{
	FFluidNiagaraDIProxyParticleRead(FIntVector2 XY) : DispatchThreadsXY(XY){}
	FIntVector2 DispatchThreadsXY;

	virtual void ConsumePerInstanceDataFromGameThread(void* Data, const FNiagaraSystemInstanceID& InstanceID) override
	{
		FFluidNDIParticleRead_GameToRenderData* IncomingData = static_cast<FFluidNDIParticleRead_GameToRenderData*>(Data);
		FFluidNDIParticleRead_RenderInstanceData* InstanceData = SystemsRenderData.Find(InstanceID);
		...
		IncomingData->~FFluidNDIParticleRead_GameToRenderData();
	}

	virtual int32 PerInstanceDataPassedToRenderThreadSize() const override
	{
		return sizeof(FFluidNDIParticleRead_GameToRenderData);
	}
	...
	void CreateRenderThreadSystemData(const FNiagaraSystemInstanceID& InstanceID, FString InDebugSourceName)
	{
		...
		FFluidNDIParticleRead_RenderInstanceData& RTInstanceData = SystemsRenderData.Add(InstanceID);
	...
	FFluidNDIParticleRead_RenderInstanceData* GetRenderDataForSystem(const FNiagaraSystemInstanceID& InstanceID)
	{
	...
	{
		const FFluidNDIParticleRead_RenderInstanceData* InstanceData = SystemsRenderData.Find(SystemInstanceID);
		if (ensure(InstanceData))
		{
			if (InstanceData->SourceEmitterGPUContext != nullptr)
			{
				if (FNiagaraDataSet* SourceDataSet = InstanceData->SourceEmitterGPUContext->MainDataSet)
				{
					...
					{
						return FIntVector(CurrentData->GetNumInstances(), DispatchThreadsXY.X, DispatchThreadsXY.Y);
					}
				}
			}
		}
...
private:
	TMap<FNiagaraSystemInstanceID, FFluidNDIParticleRead_RenderInstanceData> SystemsRenderData;
};

//////////////////////////////////////////////////////////////////////////

static void NDIParticleRead_FindAttributeIndices(FFluidNDIParticleRead_RenderInstanceData* InstanceData, const FNiagaraDataSet* SourceDataSet, const FFluidNDIParametersCS_ParticleRead& ShaderStorage)
{
	...
	for (int AttrNameIdx = 0; AttrNameIdx < ShaderStorage.AttributeNames.Num(); ++AttrNameIdx)
	{
		...
		for (int VarIdx = 0; VarIdx < SourceEmitterVariables.Num(); ++VarIdx)
		{
			...
			if (Var.GetName() == AttrName)
			{
				...
				{
					UE_LOG(LogFluidPRNiagara, Error, TEXT("Variable '%s' in emitter '%s' has type '%s', but particle read DI tried to access it as '%s'."),
		...
		if (!FoundVariable)
		{
			UE_LOG(LogFluidPRNiagara, Error, TEXT("Particle read DI is trying to access inexistent variable '%s' in emitter '%s'."), *AttrName.ToString(), *InstanceData->DebugSourceName);
			InstanceData->AttributeIndices[AttrNameIdx] = -1;
			InstanceData->AttributeCompressed[AttrNameIdx] = 0;
		}
	}
...
UFluidNiagaraDIParticleRead::UFluidNiagaraDIParticleRead(FObjectInitializer const& ObjectInitializer)
	: Super(ObjectInitializer)
{
	Proxy.Reset(new FFluidNiagaraDIProxyParticleRead(DispatchThreadsXY));
}

void UFluidNiagaraDIParticleRead::PostInitProperties()
{
...
#if WITH_EDITOR
void UFluidNiagaraDIParticleRead::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UFluidNiagaraDIParticleRead, EmitterName))
	{
...
bool UFluidNiagaraDIParticleRead::HasInternalAttributeReads(const UNiagaraEmitter* OwnerEmitter, const UNiagaraEmitter* Provider) const
{
	return Provider && Provider->GetUniqueEmitterName() == EmitterName;
}

bool UFluidNiagaraDIParticleRead::InitPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	FFluidNDIParticleRead_InstanceData* PIData = new (PerInstanceData) FFluidNDIParticleRead_InstanceData;
	...
	if (PIData->EmitterInstance == nullptr)
	{
		...
		{
			UE_LOG(LogFluidPRNiagara, Warning, TEXT("Source emitter '%s' not found. System: %s"), *EmitterName, *GetFullNameSafe(SystemInstance->GetSystem()));
		}*/
		UE_LOG(LogFluidPRNiagara, Warning, TEXT("Source emitter '%s' not found. System: %s"), *EmitterName, *GetFullNameSafe(SystemInstance->GetSystem()));
	}
	...
	FFluidNiagaraDIProxyParticleRead* ThisProxy = GetProxyAs<FFluidNiagaraDIProxyParticleRead>();
...
void UFluidNiagaraDIParticleRead::DestroyPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	FFluidNDIParticleRead_InstanceData* InstData = (FFluidNDIParticleRead_InstanceData*)PerInstanceData;
	InstData->~FFluidNDIParticleRead_InstanceData();
...
int32 UFluidNiagaraDIParticleRead::PerInstanceDataSize() const
{
	return sizeof(FFluidNDIParticleRead_InstanceData);
}

void UFluidNiagaraDIParticleRead::GetFunctions(TArray<FNiagaraFunctionSignature>& OutFunctions)
{
	// Misc functionality
	{
	...
	{
		Function.FunctionVersion = FFluidNParticleReadDIFunctionVersion::LatestVersion;
	}
#endif
}

void UFluidNiagaraDIParticleRead::GetPersistentIDFunctions(TArray<FNiagaraFunctionSignature>& OutFunctions)
{
	{
		FNiagaraFunctionSignature Sig;
...
void UFluidNiagaraDIParticleRead::GetIndexFunctions(TArray<FNiagaraFunctionSignature>& OutFunctions)
{
...
DEFINE_NDI_DIRECT_FUNC_BINDER(UFluidNiagaraDIParticleRead, GetNumSpawnedParticles);
DEFINE_NDI_DIRECT_FUNC_BINDER(UFluidNiagaraDIParticleRead, GetSpawnedIDAtIndex);
DEFINE_NDI_DIRECT_FUNC_BINDER(UFluidNiagaraDIParticleRead, GetNumParticles);
DEFINE_NDI_DIRECT_FUNC_BINDER(UFluidNiagaraDIParticleRead, GetParticleIndex);
DEFINE_NDI_DIRECT_FUNC_BINDER(UFluidNiagaraDIParticleRead, GetParticleIndexFromIDTable);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadInt);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadBool);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadFloat);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadVector2);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadVector3);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadVector4);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadColor);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadPosition);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadQuat);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadID);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadIntByIndex);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadBoolByIndex);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadFloatByIndex);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadVector2ByIndex);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadVector3ByIndex);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadVector4ByIndex);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadColorByIndex);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadPositionByIndex);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadQuatByIndex);
DEFINE_NDI_DIRECT_FUNC_BINDER_WITH_PAYLOAD(UFluidNiagaraDIParticleRead, ReadIDByIndex);
...
void UFluidNiagaraDIParticleRead::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* InstanceData, FVMExternalFunction& OutFunc)
{
	// Misc functionality
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, GetNumSpawnedParticles)::Bind(this, OutFunc);
		return;
	}

	if (BindingInfo.Name == GetIDAtSpawnIndexFunctionName)
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, GetSpawnedIDAtIndex)::Bind(this, OutFunc);
		return;
	}

	if (BindingInfo.Name == GetNumParticlesFunctionName)
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, GetNumParticles)::Bind(this, OutFunc);
		return;
	}

	if (BindingInfo.Name == GetParticleIndexFunctionName)
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, GetParticleIndex)::Bind(this, OutFunc);
		return;
	}

	if (BindingInfo.Name == GetParticleIndexFromIDTableName)
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, GetParticleIndexFromIDTable)::Bind(this, OutFunc);
		return;
	}

	bool bBindSuccessful = false;
	FFluidNDIParticleRead_InstanceData* PIData = static_cast<FFluidNDIParticleRead_InstanceData*>(InstanceData);
	...
	{
		UE_LOG(LogFluidPRNiagara, Error, TEXT("VMExternalFunction '%s' does not have a function specifier 'attribute'!"), *BindingInfo.Name.ToString());
		return;
	}
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadInt)::Bind(this, OutFunc, AttributeToRead);
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadBool)::Bind(this, OutFunc, AttributeToRead);
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadFloat)::Bind(this, OutFunc, AttributeToRead);
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadVector2)::Bind(this, OutFunc, AttributeToRead);
	...
	else if (BindingInfo.Name == GetVec3ByIDFunctionName)
	{
		...
		{
			NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadPosition)::Bind(this, OutFunc, AttributeToRead);
			bBindSuccessful = true;
		}
		else
		{
			NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadVector3)::Bind(this, OutFunc, AttributeToRead);
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadVector4)::Bind(this, OutFunc, AttributeToRead);
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadColor)::Bind(this, OutFunc, AttributeToRead);
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadQuat)::Bind(this, OutFunc, AttributeToRead);
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadID)::Bind(this, OutFunc, AttributeToRead);
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadIntByIndex)::Bind(this, OutFunc, AttributeToRead);
		bBindSuccessful = HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetIntDef());
	}
	else if (BindingInfo.Name == GetBoolByIndexFunctionName)
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadBoolByIndex)::Bind(this, OutFunc, AttributeToRead);
		bBindSuccessful = HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetBoolDef());
	}
	else if (BindingInfo.Name == GetFloatByIndexFunctionName)
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadFloatByIndex)::Bind(this, OutFunc, AttributeToRead);
		bBindSuccessful = (HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetFloatDef())
			|| HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetHalfDef()));
	}
	else if (BindingInfo.Name == GetVec2ByIndexFunctionName)
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadVector2ByIndex)::Bind(this, OutFunc, AttributeToRead);
		bBindSuccessful = (HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetVec2Def())
			|| HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetHalfVec2Def()));
	}
	else if (BindingInfo.Name == GetVec3ByIndexFunctionName)
	{
		...
		if (HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetPositionDef()))
		{
			NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadPositionByIndex)::Bind(this, OutFunc, AttributeToRead);
			bBindSuccessful = true;
		}
		else
		{
			NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadVector3ByIndex)::Bind(this, OutFunc, AttributeToRead);
			bBindSuccessful = (HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetVec3Def())
				|| HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetHalfVec3Def()));
		}
	}
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadVector4ByIndex)::Bind(this, OutFunc, AttributeToRead);
		bBindSuccessful = (HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetVec4Def())
			|| HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetHalfVec4Def()));
	}
	...
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadColorByIndex)::Bind(this, OutFunc, AttributeToRead);
		bBindSuccessful = (HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetColorDef())
			|| HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetHalfVec4Def()));
	}
	else if (BindingInfo.Name == GetQuatByIndexFunctionName)
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadQuatByIndex)::Bind(this, OutFunc, AttributeToRead);
		bBindSuccessful = (HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetQuatDef())
			|| HasMatchingVariable(EmitterVariables, AttributeToRead, FNiagaraTypeDefinition::GetHalfVec4Def()));
	}
	else if (BindingInfo.Name == GetIDByIndexFunctionName)
	{
		NDI_FUNC_BINDER(UFluidNiagaraDIParticleRead, ReadIDByIndex)::Bind(this, OutFunc, AttributeToRead);
		FNiagaraVariable VariableToRead(FNiagaraTypeDefinition::GetIDDef(), AttributeToRead);
		bBindSuccessful = EmitterVariables.Find(VariableToRead) != INDEX_NONE;
	}
	...
	if (!bBindSuccessful)
	{
		...
		UE_LOG(LogFluidPRNiagara, Warning, TEXT("ParticleRead: Failed to '%s' attribute '%s' System '%s' Emitter '%s'! Check that the attribute is named correctly."), *BindingInfo.Name.ToString(), *AttributeToRead.ToString(), *GetNameSafe(NiagaraSystem), *EmitterName);
	}
}

void UFluidNiagaraDIParticleRead::VMGetLocalSpace(FVectorVMExternalFunctionContext& Context)
{
	VectorVM::FUserPtrHandler<FFluidNDIParticleRead_InstanceData> InstData(Context);
...
void UFluidNiagaraDIParticleRead::GetNumSpawnedParticles(FVectorVMExternalFunctionContext& Context)
{
	VectorVM::FUserPtrHandler<FFluidNDIParticleRead_InstanceData> InstData(Context);
...
void UFluidNiagaraDIParticleRead::GetSpawnedIDAtIndex(FVectorVMExternalFunctionContext& Context)
{
	VectorVM::FUserPtrHandler<FFluidNDIParticleRead_InstanceData> InstData(Context);
...
void UFluidNiagaraDIParticleRead::GetNumParticles(FVectorVMExternalFunctionContext& Context)
{
	VectorVM::FUserPtrHandler<FFluidNDIParticleRead_InstanceData> InstData(Context);
...
void UFluidNiagaraDIParticleRead::GetParticleIndex(FVectorVMExternalFunctionContext& Context)
{
	VectorVM::FUserPtrHandler<FFluidNDIParticleRead_InstanceData> InstData(Context);
...
void UFluidNiagaraDIParticleRead::GetParticleIndexFromIDTable(FVectorVMExternalFunctionContext& Context)
{
	VectorVM::FUserPtrHandler<FFluidNDIParticleRead_InstanceData> InstData(Context);
...
struct FDirectReadParamHandler
{
	...
	VectorVM::FUserPtrHandler<FFluidNDIParticleRead_InstanceData> InstanceData;
...
void UFluidNiagaraDIParticleRead::ReadInt(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadWithCheck<int32>(Context, AttributeToRead, 0);
}

void UFluidNiagaraDIParticleRead::ReadBool(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadWithCheck<FNiagaraBool>(Context, AttributeToRead, FNiagaraBool(false));
}

void UFluidNiagaraDIParticleRead::ReadFloat(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadWithCheck<float>(Context, AttributeToRead, 0.0f);
}

void UFluidNiagaraDIParticleRead::ReadVector2(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadWithCheck<FVector2f>(Context, AttributeToRead, FVector2f::ZeroVector);
}

void UFluidNiagaraDIParticleRead::ReadVector3(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadWithCheck<FVector3f>(Context, AttributeToRead, FVector3f::ZeroVector);
}

void UFluidNiagaraDIParticleRead::ReadVector4(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadWithCheck<FVector4f>(Context, AttributeToRead, FVector4f(ForceInit));
}

void UFluidNiagaraDIParticleRead::ReadColor(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadWithCheck<FLinearColor>(Context, AttributeToRead, FLinearColor::White);
}

void UFluidNiagaraDIParticleRead::ReadPosition(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadWithCheck<FNiagaraPosition>(Context, AttributeToRead, FNiagaraPosition(ForceInit));
}

void UFluidNiagaraDIParticleRead::ReadQuat(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadWithCheck<FQuat4f>(Context, AttributeToRead, FQuat4f::Identity);
}

void UFluidNiagaraDIParticleRead::ReadID(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
...
void UFluidNiagaraDIParticleRead::ReadIntByIndex(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadByIndexWithCheck<int32>(Context, AttributeToRead, 0);
}

void UFluidNiagaraDIParticleRead::ReadBoolByIndex(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadByIndexWithCheck<FNiagaraBool>(Context, AttributeToRead, FNiagaraBool(false));
}

void UFluidNiagaraDIParticleRead::ReadFloatByIndex(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadByIndexWithCheck<float>(Context, AttributeToRead, 0.0f);
}

void UFluidNiagaraDIParticleRead::ReadVector2ByIndex(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadByIndexWithCheck<FVector2f>(Context, AttributeToRead, FVector2f::ZeroVector);
}

void UFluidNiagaraDIParticleRead::ReadVector3ByIndex(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadByIndexWithCheck<FVector3f>(Context, AttributeToRead, FVector3f::ZeroVector);
}

void UFluidNiagaraDIParticleRead::ReadVector4ByIndex(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadByIndexWithCheck<FVector4f>(Context, AttributeToRead, FVector4f(ForceInit));
}

void UFluidNiagaraDIParticleRead::ReadPositionByIndex(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadByIndexWithCheck<FNiagaraPosition>(Context, AttributeToRead, FNiagaraPosition(ForceInit));
}

void UFluidNiagaraDIParticleRead::ReadColorByIndex(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadByIndexWithCheck<FLinearColor>(Context, AttributeToRead, FLinearColor::White);
}

void UFluidNiagaraDIParticleRead::ReadQuatByIndex(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadByIndexWithCheck<FQuat4f>(Context, AttributeToRead, FQuat4f::Identity);
}

void UFluidNiagaraDIParticleRead::ReadIDByIndex(FVectorVMExternalFunctionContext& Context, FName AttributeToRead)
{
	ReadByIndexWithCheck<FNiagaraID>(Context, AttributeToRead, FNiagaraID());
}

bool UFluidNiagaraDIParticleRead::Equals(const UNiagaraDataInterface* Other) const
{
	if (!Super::Equals(Other))
	{
		return false;
	}
	return CastChecked<UFluidNiagaraDIParticleRead>(Other)->EmitterName == EmitterName;
}

bool UFluidNiagaraDIParticleRead::CopyToInternal(UNiagaraDataInterface* Destination) const
{
	if (!Super::CopyToInternal(Destination))
	{
		return false;
	}
	CastChecked<UFluidNiagaraDIParticleRead>(Destination)->EmitterName = EmitterName;
	return true;
}

#if WITH_EDITORONLY_DATA
bool UFluidNiagaraDIParticleRead::AppendCompileHash(FNiagaraCompileHashVisitor* InVisitor) const
{
...
void UFluidNiagaraDIParticleRead::GetParameterDefinitionHLSL(const FNiagaraDataInterfaceGPUParamInfo& ParamInfo, FString& OutHLSL)
{
...
static bool GenerateGetFunctionHLSL(const FNiagaraDataInterfaceGPUParamInfo& ParamInfo, const FNiagaraDataInterfaceGeneratedFunction& FunctionInfo, int FunctionInstanceIndex, ENiagaraParticleDataComponentType ComponentType, int NumComponents, bool bByIndex, FString& OutHLSL)
{
	...
	if (ComponentType != ENiagaraParticleDataComponentType::Float)
	{
		...
		case ENiagaraParticleDataComponentType::ID:
			...
			break;
		default:
			UE_LOG(LogFluidPRNiagara, Error, TEXT("Unknown component type %d while generating function %s"), ComponentType, *FunctionInfo.InstanceName);
			return false;
		}
...
bool UFluidNiagaraDIParticleRead::GetFunctionHLSL(const FNiagaraDataInterfaceGPUParamInfo& ParamInfo, const FNiagaraDataInterfaceGeneratedFunction& FunctionInfo, int FunctionInstanceIndex, FString& OutHLSL)
{
	//
	// Spawn info and particle count
	//
...
void UFluidNiagaraDIParticleRead::BuildShaderParameters(FNiagaraShaderParametersBuilder& ShaderParametersBuilder) const
{
...
void UFluidNiagaraDIParticleRead::SetShaderParameters(const FNiagaraDataInterfaceSetShaderParametersContext& Context) const
{
	FFluidNiagaraDIProxyParticleRead& DIProxy = Context.GetProxy<FFluidNiagaraDIProxyParticleRead>();

	const FFluidNDIParametersCS_ParticleRead& ShaderStorage = Context.GetShaderStorage<FFluidNDIParametersCS_ParticleRead>();
	...
	FFluidNDIParticleRead_RenderInstanceData* InstanceData = DIProxy.GetRenderDataForSystem(Context.GetSystemInstanceID());
	...
	if (InstanceData->SourceEmitterGPUContext == nullptr)
	{
		...
		{
			UE_LOG(LogFluidPRNiagara, Error, TEXT("GPU particle read DI is set to access CPU emitter '%s'."), *InstanceData->DebugSourceName);
	...
	if (Context.GetComputeInstanceData().Context == InstanceData->SourceEmitterGPUContext)
	{
		...
		if (const FSimulationStageMetaData* StageMetaData = Context.GetSimStageData().StageMetaData)
		{
			...
			{
				UE_LOG(LogFluidPRNiagara, Error, TEXT("Particle read DI reading self '%s' on stage '%s' is unsafe, please disable partial writes on the stage."), *InstanceData->DebugSourceName, *StageMetaData->SimulationStageName.ToString());
				SetErrorParams(true);
				return;
			}
		}
	...
	if (InstanceData->bWarnFailedToFindAcquireTag && (InstanceData->AcquireTagRegisterIndex == -1) && Context.IsParameterBound(&ShaderParameters->AcquireTagRegisterIndex))
	{
		...
		UE_LOG(LogFluidPRNiagara, Error, TEXT("Particle read DI cannot find ID variable in emitter '%s'."), *InstanceData->DebugSourceName);
	}
}

FNiagaraDataInterfaceParametersCS* UFluidNiagaraDIParticleRead::CreateShaderStorage(const FNiagaraDataInterfaceGPUParamInfo& ParameterInfo, const FShaderParameterMap& ParameterMap) const
{
	FFluidNDIParametersCS_ParticleRead* ShaderStorage = new FFluidNDIParametersCS_ParticleRead();
...
const FTypeLayoutDesc* UFluidNiagaraDIParticleRead::GetShaderStorageType() const
{
	return &StaticGetTypeLayoutDesc<FFluidNDIParametersCS_ParticleRead>();
}

void UFluidNiagaraDIParticleRead::ProvidePerInstanceDataForRenderThread(void* DataForRenderThread, void* PerInstanceData, const FNiagaraSystemInstanceID& SystemInstance)
{
	FFluidNDIParticleRead_GameToRenderData* RTData = new (DataForRenderThread) FFluidNDIParticleRead_GameToRenderData;
	FFluidNDIParticleRead_InstanceData* PIData = static_cast<FFluidNDIParticleRead_InstanceData*>(PerInstanceData);
...
bool UFluidNiagaraDIParticleRead::UpgradeFunctionCall(FNiagaraFunctionSignature& FunctionSignature)
{
	bool bWasChanged = false;

	// Early out for version matching
	if (FunctionSignature.FunctionVersion == FFluidNParticleReadDIFunctionVersion::LatestVersion)
	{
		return bWasChanged;
	}

	// Renamed some functions
	if (FunctionSignature.FunctionVersion < FFluidNParticleReadDIFunctionVersion::RenamedSpawnIndex)
	{
	...
	// Set latest version
	FunctionSignature.FunctionVersion = FFluidNParticleReadDIFunctionVersion::LatestVersion;
...
void UFluidNiagaraDIParticleRead::GetFeedback(UNiagaraSystem* Asset, UNiagaraComponent* Component, TArray<FNiagaraDataInterfaceError>& OutErrors, TArray<FNiagaraDataInterfaceFeedback>& Warnings, TArray<FNiagaraDataInterfaceFeedback>& Info)
{
	if (!Asset)
	{
		return;
...
void UFluidNiagaraDIParticleRead::GetEmitterDependencies(UNiagaraSystem* Asset, TArray<FVersionedNiagaraEmitter>& Dependencies) const
{
	if (!Asset)
	{
		return;
...
bool UFluidNiagaraDIParticleRead::ReadsEmitterParticleData(const FString& InEmitterName) const
{
