...
// #include UE_INLINE_GENERATED_CPP_BY_NAME(FluidNiagaraDI_RT2D)
// 将生成的 cpp 文件内联到模块 cpp 文件的支持。这缩短了编译时间，因为需要更少的标头解析。

#define LOCTEXT_NAMESPACE "FluidNiagaraDI_RT2D"

namespace NDIFluidRenderTarget2DLocal
{
	BEGIN_SHADER_PARAMETER_STRUCT(FShaderParameters, )
		...
		static const TCHAR* TemplateShaderFile = TEXT("/Plugin/FluidDynamic/Private/FluidNiagaraDI_RT2D.usf");
...
FNiagaraVariableBase UFluidNiagaraDI_RT2D::ExposedRTVar;
/*--------------------------------------------------------------------------------------------------------------------------*/

struct FNDIFluidRenderTarget2DFunctionVersion
{
	enum Type
	{
		InitialVersion = 0,
...
#if STATS
void FFluidRenderTarget2DRWInstanceData_RenderThread::UpdateMemoryStats()
{
...
UFluidNiagaraDI_RT2D::UFluidNiagaraDI_RT2D(FObjectInitializer const& ObjectInitializer)
	: Super(ObjectInitializer)
{
	Proxy.Reset(new FFluidNiagaraDataInterfaceProxyRenderTarget2DProxy());

	FNiagaraTypeDefinition Def(UTextureRenderTarget::StaticClass());
	RenderTargetUserParameter.Parameter.SetType(Def);
}

void UFluidNiagaraDI_RT2D::PostInitProperties()
{
...
void UFluidNiagaraDI_RT2D::GetFunctions(TArray<FNiagaraFunctionSignature>& OutFunctions)
{
	using namespace NDIFluidRenderTarget2DLocal;
...

	{
		...
		Sig.FunctionVersion = FNDIFluidRenderTarget2DFunctionVersion::LatestVersion;
#endif
	}
...
#if WITH_EDITORONLY_DATA
bool UFluidNiagaraDI_RT2D::UpgradeFunctionCall(FNiagaraFunctionSignature& FunctionSignature)
{
	using namespace NDIFluidRenderTarget2DLocal;

	bool bWasChanged = false;

	if (FunctionSignature.FunctionVersion < FNDIFluidRenderTarget2DFunctionVersion::AddedOptionalExecute)
	{
	...
	// Set latest version
	FunctionSignature.FunctionVersion = FNDIFluidRenderTarget2DFunctionVersion::LatestVersion;

	return bWasChanged;
}
#endif

DEFINE_NDI_DIRECT_FUNC_BINDER(UFluidNiagaraDI_RT2D, VMGetSize);
DEFINE_NDI_DIRECT_FUNC_BINDER(UFluidNiagaraDI_RT2D, VMSetSize);
void UFluidNiagaraDI_RT2D::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* InstanceData, FVMExternalFunction& OutFunc)
{
	using namespace NDIFluidRenderTarget2DLocal;
	...
	if (BindingInfo.Name == GetSizeFunctionName)
	{
		...
		NDI_FUNC_BINDER(UFluidNiagaraDI_RT2D, VMGetSize)::Bind(this, OutFunc);
	}
	else if (BindingInfo.Name == SetSizeFunctionName)
	{
		check(BindingInfo.GetNumInputs() == 3 && BindingInfo.GetNumOutputs() == 1);
		NDI_FUNC_BINDER(UFluidNiagaraDI_RT2D, VMSetSize)::Bind(this, OutFunc);
	}

}

bool UFluidNiagaraDI_RT2D::Equals(const UNiagaraDataInterface* Other) const
{
	...
	const UFluidNiagaraDI_RT2D* OtherTyped = CastChecked<const UFluidNiagaraDI_RT2D>(Other);
	return
		OtherTyped != nullptr &&
#if WITH_EDITORONLY_DATA
...
bool UFluidNiagaraDI_RT2D::CopyToInternal(UNiagaraDataInterface* Destination) const
{
	...
	UFluidNiagaraDI_RT2D* DestinationTyped = CastChecked<UFluidNiagaraDI_RT2D>(Destination);
	if (!DestinationTyped)
	{
		return false;
	}
...
#if WITH_EDITORONLY_DATA
bool UFluidNiagaraDI_RT2D::AppendCompileHash(FNiagaraCompileHashVisitor* InVisitor) const
{
	bool bSuccess = Super::AppendCompileHash(InVisitor);
	FSHAHash Hash = GetShaderFileHash(NDIFluidRenderTarget2DLocal::TemplateShaderFile, EShaderPlatform::SP_PCD3D_SM5);
	InVisitor->UpdateString(TEXT("UFluidNiagaraDI_RT2DTemplateHLSLSource"), Hash.ToString());
	InVisitor->UpdateShaderParameters<NDIFluidRenderTarget2DLocal::FShaderParameters>();
	return bSuccess;
}

void UFluidNiagaraDI_RT2D::GetParameterDefinitionHLSL(const FNiagaraDataInterfaceGPUParamInfo& ParamInfo, FString& OutHLSL)
{
	...
	LoadShaderSourceFile(NDIFluidRenderTarget2DLocal::TemplateShaderFile, EShaderPlatform::SP_PCD3D_SM5, &TemplateFile, nullptr);
	OutHLSL += FString::Format(*TemplateFile, TemplateArgs);
}

bool UFluidNiagaraDI_RT2D::GetFunctionHLSL(const FNiagaraDataInterfaceGPUParamInfo& ParamInfo, const FNiagaraDataInterfaceGeneratedFunction& FunctionInfo, int FunctionInstanceIndex, FString& OutHLSL)
{
	using namespace NDIFluidRenderTarget2DLocal;
...
void UFluidNiagaraDI_RT2D::BuildShaderParameters(FNiagaraShaderParametersBuilder& ShaderParametersBuilder) const
{
	ShaderParametersBuilder.AddNestedStruct<NDIFluidRenderTarget2DLocal::FShaderParameters>();
}

void UFluidNiagaraDI_RT2D::SetShaderParameters(const FNiagaraDataInterfaceSetShaderParametersContext& Context) const
{
	FFluidNiagaraDataInterfaceProxyRenderTarget2DProxy& DIProxy = Context.GetProxy<FFluidNiagaraDataInterfaceProxyRenderTarget2DProxy>();
	FFluidRenderTarget2DRWInstanceData_RenderThread* InstanceData_RT = DIProxy.SystemInstancesToProxyData_RT.Find(Context.GetSystemInstanceID());
	...
	NDIFluidRenderTarget2DLocal::FShaderParameters* Parameters = Context.GetParameterNestedStruct<NDIFluidRenderTarget2DLocal::FShaderParameters>();
...
bool UFluidNiagaraDI_RT2D::InitPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	check(Proxy);

	FFluidRenderTarget2DRWInstanceData_GameThread* InstanceData = new (PerInstanceData) FFluidRenderTarget2DRWInstanceData_GameThread();
...
void UFluidNiagaraDI_RT2D::DestroyPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	SystemInstancesToProxyData_GT.Remove(SystemInstance->GetId());

	FFluidRenderTarget2DRWInstanceData_GameThread* InstanceData = static_cast<FFluidRenderTarget2DRWInstanceData_GameThread*>(PerInstanceData);

	InstanceData->~FFluidRenderTarget2DRWInstanceData_GameThread();

	FFluidNiagaraDataInterfaceProxyRenderTarget2DProxy* RT_Proxy = GetProxyAs<FFluidNiagaraDataInterfaceProxyRenderTarget2DProxy>();
	ENQUEUE_RENDER_COMMAND(FNiagaraDIDestroyInstanceData) (
		[RT_Proxy, InstanceID = SystemInstance->GetId()](FRHICommandListImmediate& CmdList)
	{
#if STATS
		if (FFluidRenderTarget2DRWInstanceData_RenderThread* TargetData = RT_Proxy->SystemInstancesToProxyData_RT.Find(InstanceID))
		{
...
void UFluidNiagaraDI_RT2D::GetExposedVariables(TArray<FNiagaraVariableBase>& OutVariables) const
{
	OutVariables.Emplace(ExposedRTVar);
}

bool UFluidNiagaraDI_RT2D::GetExposedVariableValue(const FNiagaraVariableBase& InVariable, void* InPerInstanceData, FNiagaraSystemInstance* InSystemInstance, void* OutData) const
{
	FFluidRenderTarget2DRWInstanceData_GameThread* InstanceData = static_cast<FFluidRenderTarget2DRWInstanceData_GameThread*>(InPerInstanceData);
...
void UFluidNiagaraDI_RT2D::GetCanvasVariables(TArray<FNiagaraVariableBase>& OutVariables) const
{
...
bool UFluidNiagaraDI_RT2D::RenderVariableToCanvas(FNiagaraSystemInstanceID SystemInstanceID, FName VariableName, class FCanvas* Canvas, const FIntRect& DrawRect) const
{
	if (!Canvas)
	{
		return false;
	...
	FFluidRenderTarget2DRWInstanceData_GameThread* GT_InstanceData = SystemInstancesToProxyData_GT.FindRef(SystemInstanceID);
	if (!GT_InstanceData)
	{
		return false;
	}
...
void UFluidNiagaraDI_RT2D::VMSetSize(FVectorVMExternalFunctionContext& Context)
{
	// This should only be called from a system or emitter script due to a need for only setting up initially.
	VectorVM::FUserPtrHandler<FFluidRenderTarget2DRWInstanceData_GameThread> InstData(Context);
...
void UFluidNiagaraDI_RT2D::VMGetSize(FVectorVMExternalFunctionContext& Context)
{
	VectorVM::FUserPtrHandler<FFluidRenderTarget2DRWInstanceData_GameThread> InstData(Context);
...
bool UFluidNiagaraDI_RT2D::PerInstanceTick(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	FFluidRenderTarget2DRWInstanceData_GameThread* InstanceData = static_cast<FFluidRenderTarget2DRWInstanceData_GameThread*>(PerInstanceData);
	...
	if (bInheritUserParameterSettings)
	{
		...
		{
			UE_LOG(LogFluidFrid2DNiagara, Error, TEXT("RenderTarget UserParam is required but invalid."));
		}
	}
...
bool UFluidNiagaraDI_RT2D::PerInstanceTickPostSimulate(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	if (bUpdateRT)
	{
		...
		{
			FFluidRenderTarget2DRWInstanceData_RenderThread* TargetData = &RT_Proxy->SystemInstancesToProxyData_RT.FindOrAdd(RT_InstanceID);
...
void FFluidNiagaraDataInterfaceProxyRenderTarget2DProxy::PostStage(const FNDIGpuComputePostStageContext& Context)
{
	if (FFluidRenderTarget2DRWInstanceData_RenderThread* ProxyData = SystemInstancesToProxyData_RT.Find(Context.GetSystemInstanceID()))
	{
...
void FFluidNiagaraDataInterfaceProxyRenderTarget2DProxy::PostSimulate(const FNDIGpuComputePostSimulateContext& Context)
{
	FFluidRenderTarget2DRWInstanceData_RenderThread* ProxyData = SystemInstancesToProxyData_RT.Find(Context.GetSystemInstanceID());
...
FIntVector FFluidNiagaraDataInterfaceProxyRenderTarget2DProxy::GetElementCount(FNiagaraSystemInstanceID SystemInstanceID) const
{
	if (const FFluidRenderTarget2DRWInstanceData_RenderThread* TargetData = SystemInstancesToProxyData_RT.Find(SystemInstanceID))
	{
